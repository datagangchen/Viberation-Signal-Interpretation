function [val_opt, Popt, status, nb_call_total]  = GpOptimPropNM(Sys, P, phi, opt)
%SOPTIMPROP optimizes the satisfaction of a property with Gaussian process
%optimization method
% 
% Synopsis: [val_opt, Popt, status] = GpOptimProp(Sys, P, phi, opt)
% 
% Inputs:
%  - Sys : is a system
...
for ii = iv(1:Ninit)
    ...
    switch opt.method
      ...
      [~, val_opt(kk)] = optimize(fun,x0,lbound,ubound,[],[],[],[],[],[],options,'NelderMead');
        ...
        [Xtnew,Ytnew,Sigma] = ActiveGpOptHyper( fun,Xt, Yt, Xs,25,'opthyper',false,'algo', 'gpacb');
         val_opt(kk)=-max(Ytnew);
        case 'gpucb'
        [ Xs, Xt, Yt] = gp_sample_hyper(fun,lbound,ubound );
        [Xtnew,Ytnew,Sigma] = ActiveGpOptHyper( fun,Xt, Yt, Xs,25,'opthyper',false,'algo', 'gpucb');
         val_opt(kk)=-max(Ytnew);
        case 'gpacbhyper'
        [ Xs, Xt, Yt] = gp_sample_hyper(fun,lbound,ubound);
        [Xtnew,Ytnew,Sigma] = ActiveGpOptHyper( fun,Xt, Yt, Xs,25,'opthyper',true,'algo', 'gpacb');
         val_opt(kk)=-max(Ytnew);
        case 'gpucbhyper'
         [ Xs, Xt, Yt] = gp_sample_hyper(fun,lbound,ubound);
        [Xtnew,Ytnew,Sigma] = ActiveGpOptHyper( fun,Xt, Yt, Xs,25,'opthyper',true,'algo', 'gpucb');
         val_opt(kk)=-max(Ytnew);          
        otherwise
        fprintf('\nPlease choose a defined method.\n'); 
    end
