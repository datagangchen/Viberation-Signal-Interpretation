function [val__, tau__] = QMITL_EvalClassic(Sys, phi, P, trajs, t,dt__)
%QMITL_EVALCLASSIC computes the satisfaction function of a property for one
...
%
% Synopsis: [val__, tau__] = QMITL_Eval(Sys, phi, P, trajs[, t,dt])
% 
% Inputs:
%  - Sys   : the system
%  - phi   : STL formula
...
if ischar(phi)
    ...
    [val__, tau__, status__] = QMITL_EvalClassic(Sys, phi_tmp__, P, trajs, t);
...
try %#ok<TRYNC>
    if BreachGlobOpt.AltEval
        warning('QMITL_EvalClassic:otherMethod',...
            'QMITL_EvalClassic is called, but as the system global variable AltEval is true, QMITL_EvalThom is used.');
        [val__, tau__, status__] = QMITL_EvalThom(Sys, phi, P, trajs, t);
        return;
    end
end
...
exist_t = exist('t','var'); % boolean indicating if t is provided as arg of QMITL_EvalClassic
