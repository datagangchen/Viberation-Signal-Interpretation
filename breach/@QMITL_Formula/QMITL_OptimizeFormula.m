function phi = QMITL_OptimizeFormula(phi)
%QMITL_OptimizeFormula optimizes recursivly a formula.
% 
% Synopsis: phi = QMITL_OptimizeFormula(phi)
% 
% Input:
...
if strcmp(phi.type,'andn')
    ...
    if(updated) % if some changes are done, we apply the function again (if some subformula in phin are "and" or "andn")
        phi = QMITL_OptimizeFormula(phi);
    else
        for ii=1:numel(phi.phin)
            phi.phin(ii) = QMITL_OptimizeFormula(phi.phin(ii)); % otherwize, we go down in the tree
        end
    end
...
elseif strcmp(phi.type,'and') % we concatenate only one subformula (if appliable)
    if strcmp(phi.phi1.type,'and')
        ...
        phi.phi2 = [];
        phi = QMITL_OptimizeFormula(phi); % apply the function again (if some subformula in phin are "and" or "andn")
    ...
    elseif strcmp(phi.phi2.type,'andn')
        ...
        phi.phi1 = QMITL_OptimizeFormula(phi.phi1);
        phi.phi2 = QMITL_OptimizeFormula(phi.phi2);
        
    end
    
else % neither an "and" or an "andn", so optimize subformula
    if ~isempty(phi.phi)
        phi.phi = QMITL_OptimizeFormula(phi.phi);
